...
def find_start(
    ...
    found = found if found is None else found.span()[0]
    return found
...
def has_shine_dalgarno(
    ...
    if search_start < 0:
        return False
    for match in shine_regex.finditer(sequence, search_start, search_stop):
        return True  # Found the shine_dalgarno sequence
    return False
...
def predict_genes(
    ...
    while sequence_length - current_position >= min_gap:
        start = find_start(start_regex, sequence, current_position, sequence_length)
        if start is not None:
            stop = find_stop(stop_regex, sequence, start)
            if stop is not None:
                if stop - start >= min_gene_len:
            ...
            break  # No more start codons, exit the loop
...
def main() -> None:  # pragma: no cover
    ...
    probable_genes_comp = sorted(
        [
            [len(sequence) - pos_j + 1, len(sequence) - pos_i + 1]
            for pos_i, pos_j in probable_genes_comp
        ],
        key=lambda kv: kv[0],
    )
    # Adjusting positions of predicted genes in 3' to 5' orientation to be represented in 5' to 3' orientation
    # for gene in probable_genes_comp:
    # start, stop = gene
    # gene[0] = sequence_length - stop + 1
    # gene[1] = sequence_length - start + 1
    # Call to output functions
